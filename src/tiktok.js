...
export async function publishCarouselToTikTok({ imageUrls, imagePaths, title, description }) {
  ...
  // Images-only policy: never convert the carousel into a slideshow video.
  // TikTok photo carousels require public image URLs, so if they are not
  // ready we fail loudly instead of silently posting a video.
  if (!hasUrls) {
    throw new Error("TikTok photo carousel butuh minimal 2 public image URLs (slideshow video dinonaktifkan).");
  }
